...
app.post('/add-goods', (req, res) => {
    try {
        let obj = req.body
        myBag.push(obj)
        res.status(201).send(`Element ${obj.product_name} was created`)
    } catch (err) {
        res.status(410).send('DO NOT WORK')
    }
})
...
app.delete('/delete-mybag/:id', (req, res) => {
    ...
    myBag.forEach((item) => {
        if (id === item.id) {
            res.send(`Element with ${item.product_name} was deleted from bag`)
        }
    });
...
app.put('/change-goods/:id', (req, res) => {
    try {
        ...
        res.status(201).send(`Element with id ${id} was changed`)
    } catch (err) {
        res.status(410).send('DO NOT WORK')
    }
})
