...
export function reducer(state = initialState, action) {
  if (action.type === 'DELETE PRODUCT') {
    ...
    let newArr = state.myArr.filter((item) => state.myArr.indexOf(item) !== index);
    return {...state, myArr: newArr};
  } else if (action.type === 'UPDATE PRODUCT') {
    let index = action.payload.index;
    let newArr = state.myArr.map((item) => {
      if (state.myArr.indexOf(item) === index) {
        item.product_name = action.payload.name;
        item.product_description = action.payload.description;
        item.product_price = action.payload.price;
      }
      return item;
    });
    return {...state, myArr: newArr};
  } else if (action.type === 'CREATE PRODUCT') {
    let newArr = [...state.myArr];
    let newProduct = {
      product_name: action.payload.name,
      product_description: action.payload.description,
